...
<head>
  ...
  <style>
    ...
    table.board-table td:nth-child(2) { text-align: left; padding-left: 20px; width: 50%; }
    ...
    .question-btn { display: flex; justify-content: flex-end; margin-top: 30px; }
    .question-btn button { background-color: #111; border: 1px solid #A983A3; color: #A983A3; padding: 10px 24px; border-radius: 8px; cursor: pointer; }
  </style>
</head>
...
<div class="main-content">
  ...
  <div class="filters">
    <div></div>
    <c:choose>
      <c:when test="${not empty loginUser and loginUser.userAuthority != '2'}">
  ...
  <div class="board-footer">
    <div class="pagination">
      <c:if test="${currentPage > 1}">
        <button onclick="location.href='qnaList.do?page=${currentPage - 1}'">&lt;</button>
      </c:if>
      <c:forEach begin="1" end="${totalPages}" var="pageNum">
        <button onclick="location.href='qnaList.do?page=${pageNum}'" class="${pageNum == currentPage ? 'selected' : ''}">
          ${pageNum}
        </button>
      </c:forEach>
