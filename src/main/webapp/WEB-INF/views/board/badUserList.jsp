...
<head>
  ...
  <style>
    ...
    td a { color: #A983A3; text-decoration: underline; }
...
<div class="main-content">
  ...
  <c:choose>
    <c:when test="${not empty badUserList}">
      <c:forEach var="user" items="${badUserList}">
        <tr>
          ...
          <td>
            <c:choose>
              <c:when test="${not empty user['POST_TITLE']}">
                <a href="postDetail.do?postId=${user['POST_ID']}">
                  ${user['POST_TITLE']}
                </a>
              </c:when>
              <c:otherwise>-</c:otherwise>
          ...
          <td><input type="checkbox" name="selectedUsers" value="${user['USER_ID']}" /></td>
        </tr>
      </c:forEach>
    </c:when>
    <c:otherwise>
      <tr><td colspan="5" class="empty-message">불량 이용자가 없습니다.</td></tr>
    </c:otherwise>
  </c:choose>
</tbody>
    </table>
    ...
    <div class="bottom-bar">
      ...
      <div class="right">
        <button type="submit" class="btn-action" formaction="restoreBadUsers.do">불량 이용자 등록 해제</button>
        <button type="submit" class="btn-action btn-danger" formaction="withdrawBadUsers.do">탈퇴시키기</button>
      </div>
    </div>
  </form>
